...
var rotateLight, rotateLight2, rotateNode, wheels;
var light2Animation, wheelsAnimation, animation;
...
loadResources({
  ...
  car: 'models/car.obj',
  chassis: 'models/car_chassis.obj',
  wheels:  'models/car_4_wheels.obj',
  wheel:  'models/car_1_wheel.obj',
  wheel_origin: 'models/car_1_wheel_origin.obj'
...
function createSceneGraph(gl, resources) {
...

  {
    ...
    animation = new Animation(rotateLight2,
      [{matrix: glm.transform({translate: [0, 0, 0], rotateX: 1, rotateY: 1, rotateZ: 1}), duration: 0.1},
       {matrix: glm.transform({translate: [1, 2, 0], rotateX: 1, rotateY: 1, rotateZ: 1}), duration: 2000},
       {matrix: glm.transform({translate: [0, -1, 0], rotateX: 1, rotateY: 1, rotateZ: 1}), duration: 2000},
       {matrix: glm.transform({translate: [3, 2, 0], rotateX: 1, rotateY: 1, rotateZ: 1}), duration: 2000}],
      true
    );
...

  {
    ...
    let chassis = new MaterialNode([
      new RenderSGNode(resources.chassis)
    ]);
    //gold
    ...
    c3po.shininess = 50;*/
    chassis.ambient = [1, 1, 1, 1];
    chassis.diffuse = [0.035652, 0.003963, 0.800000, 1];
    chassis.specular = [0.500000, 0.500000, 0.500000, 1];
    chassis.emission = [0, 0, 0, 1];
    chassis.shininess = 50;

    
    wheels = new TransformationSGNode(mat4.create(),//glm.transform({translate: [0, 0, 0], rotateX: 90, rotateY: 1, rotateZ: 1}),
          new MaterialNode([
              new RenderSGNode(resources.wheel)]
          )
    );

 /*    wheelsAnimation = new Animation(wheels, [
      { matrix: progress => mat4.rotateX(
            mat4.create(), //output
            mat4.translate(mat4.create(), mat4.create(), [0, 0, 0]), // matrix to tanslate
            glm.deg2rad(-360 * progress) //translation
          ), duration: 1000 },
    ...
    /*var upWheel = new TransformationSGNode(mat4.multiply(mat4.create(), wheels.matrix, glm.transform({translate: [0, 0, 0], rotateX: 90, rotateY: 1, rotateZ: 1})),
      wheels
    );*/

    animation2 = new Animation(wheels,
      [
       {matrix: glm.transform({translate: [0, 0, 0], rotateX: 1, rotateY: 1, rotateZ: 180}),    duration: 3000},
       //{matrix: glm.transform({translate: [0, 0, 0], rotateX: 1, rotateY: 1, rotateZ: 270}),   duration: 5000},
      // {matrix: glm.transform({translate: [0, 0, 0], rotateX: 0, rotateY: 0, rotateZ: 180}),  duration: 2000},
       //{matrix: glm.transform({translate: [0, 0, 0], rotateX: 0, rotateY: 0, rotateZ: 270}),  duration: 2000},
       //{matrix: glm.transform({translate: [0, 0, 0], rotateX: 0, rotateY: 0, rotateZ: 360}),  duration: 2000}
      ],
      true
    ); 
    ...
    /*var elevateYNode = new TransformationSGNode(mat4.create(),[
      new TransformationSGNode(glm.translate(0,0,0), [
        chassis, wheels
      ])
    ]);*/
      
    //root.append(elevateYNode);
    //animation.start();
    root.append(wheels);
    //light2Animation.start();
...
function render(timeInMilliseconds) {
  ...
  //wheels.matrix = glm.rotateZ(timeInMilliseconds*0.05);
  //TASK 5-2 enable light rotation
  light2Animation.update(deltaTime);
  //animation.update(deltaTime);
  animation2.update(deltaTime);
