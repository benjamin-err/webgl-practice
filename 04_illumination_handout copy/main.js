...
function createSceneGraph(gl, resources) {
...

  {
    ...
    let car_obj = new MaterialNode([
      new RenderSGNode(resources.car)
    ]);

     car_obj.ambient = [0.24725, 0.1995, 0.0745, 1];
    car_obj.diffuse = [0.75164, 0.60648, 0.22648, 1];
    car_obj.specular = [0.628281, 0.555802, 0.366065, 1];
    car_obj.shininess = 50;
 

    /*car_obj.ambient   = [0,0,0,255].map(x => x / 255);
    car_obj.diffuse   = [26,89,26,255].map(x => x / 255);
    car_obj.specular  = [115,140,115,255].map(x => x / 255);
    car_obj.shininess = 32;*/

    rotateNode = new TransformationSGNode(mat4.create(), [
      new TransformationSGNode(glm.transform({ translate: [0,-1,0], rotateY: 45, scale: 1}),  [
        car_obj
      ])
    ]);
...

   {
    ...
    root.append(new TransformationSGNode(glm.transform({ translate: [0,-3,0], rotateX: -90, scale: 3}), [
      floor
    ]));
  } 
